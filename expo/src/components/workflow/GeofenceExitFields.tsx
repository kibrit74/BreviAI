...
import { GeofenceExitConfig } from '../../types/workflow-types';
import { GeofenceService } from '../../services/GeofenceService';

interface GeofenceExitFieldsProps {
    config: GeofenceExitConfig;
    updateConfig: (key: string, value: any) => void;
}

export const GeofenceExitFields: React.FC<GeofenceExitFieldsProps> = ({ config, updateConfig }) => {
        ...
        <View>
                        ...
                        availableGeofences.map((geofence) => (
                            <TouchableOpacity
                                ...
                                style={{
                                    ...
                                    borderRadius: 8,
                                    borderWidth: 1,
                                    borderColor: config.geofenceId === geofence.id ? '#EF4444' : '#ddd',
                                    backgroundColor: config.geofenceId === geofence.id ? '#EF444420' : '#f9f9f9'
                                }}
                                onPress={() => updateConfig('geofenceId', geofence.id)}
                            >
                                <Text style={{
                                    color: config.geofenceId === geofence.id ? '#EF4444' : '#333',
                                    fontSize: 16,
                                    fontWeight: '500'
                                }}>
                                    📍 {geofence.name}
            ...
            <View style={{ marginBottom: 16 }}>
                ...
                <TextInput
                    ...
                    placeholder="geofence_exit_event"
                    placeholderTextColor="#999"
                />
                <Text style={{ fontSize: 12, color: '#666', marginTop: 4 }}>
                    Çıkış event verileri bu değişkene kaydedilir
                </Text>
            </View>
            ...
            <View style={{ marginBottom: 16, backgroundColor: '#F3F4F6', padding: 12, borderRadius: 8 }}>
                ...
                <Text style={{ fontSize: 14, color: '#6B7280' }}>
                    Bu trigger, seçilen coğrafi sınırdan çıkıldığında otomatik olarak çalışır.{'\n'}
